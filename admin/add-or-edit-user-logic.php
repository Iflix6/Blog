...
if (isset($_POST['submit']) &&  in_array($mode, $valid_mode)) {
  ...
  if ($_SESSION[$mode]) {
    ...
    if ($mode == 'add-user') {
      $query = "INSERT INTO user SET firstname='$firstname', lastname='$lastname', username='$username', email='$email', password='$hashed_password', avatar='$avatar_name', is_admin=$is_admin";
    } elseif ($mode == 'edit-user') {
      $query = "UPDATE user SET firstname='$firstname', lastname='$lastname', username='$username', email='$email', is_admin=$is_admin WHERE id=$id LIMIT 1";
    }
